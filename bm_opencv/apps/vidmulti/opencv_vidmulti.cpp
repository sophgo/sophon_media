...
// #define DISPLAY_THREAD_FRAMERATE
...
#endif
{
    ...
    int reopen_flag = 0;
    char *reopen = getenv("REOPEN_FLAG");
    if (reopen) reopen_flag = atoi(reopen);
    ...
    while (!g_exit_flag) {
        ...
        if (g_dump_flag[id]) {
            ...
            frame_idx++;
#ifdef HAVE_BMCV
            imwrite("out-" + std::to_string(id)  + "-" + std::to_string(pts) + "idx" + std::to_string(frame_idx) + ".jpg", frame);
#else
            imwrite("out-" + std::to_string(id)  + "-" + std::to_string(pts) + "idx" + std::to_string(frame_idx) + ".jpg", image);
#endif
        }
...
void usages(char **argv)
{
    ...
    cout << "set env for display:" << "[export VIDMULTI_DISPLAY_FRAMERATE=0]   display only frames. " << endl;
    cout << "                    "  << "[export VIDMULTI_DISPLAY_FRAMERATE=1]  display frame rate for each channel." << endl;
#else
    ...
    cout << "                    "   << "[export VIDMULTI_DISPLAY_FRAMERATE=1]  display frame rate for each channel." << endl;
#endif
    cout << "set reopen model:"  << "[export REOPEN_FLAG=0]   close reopen. " << endl;
    cout << "            "   << "[export REOPEN_FLAG=1]  reopen every 400 frames." << endl;
}
