...
gst_v4l2_memory_group_new (GstV4l2Allocator * allocator, guint32 index)
{
  ...
  if (V4L2_TYPE_IS_MULTIPLANAR (format->type)) {
    ...
    GST_INFO_OBJECT (allocator, "[QUERYBUF] type:%d, index:%d, memory:%d",
      group->buffer.type, group->buffer.index, group->buffer.memory);
  ...
  for (i = 0; i < group->n_mem; i++) {
    ...
    //  GstV4l2Memory *mem;
      ...
      GST_INFO_OBJECT (allocator,
          "mmap buffer[%d] length %d, data offset %d, plane %d mmap_offset:%d",
          group->buffer.index,
      ...
      // GST_INFO_OBJECT (allocator, "exported DMABUF[%d] as fd %d", expbuf.index, expbuf.fd);
      // group->mem[i] = (GstMemory *) _v4l2mem_new (0, GST_ALLOCATOR (allocator),
...
GstV4l2MemoryGroup *
gst_v4l2_allocator_remake_dma_buf (GstV4l2Allocator * allocator,
                               GstAllocator * dmabuf_allocator, guint index)
{
  GstV4l2Object *obj = allocator->obj;
  GstV4l2MemoryGroup *group;
  guint i;

  group = allocator->groups[index];
  ...
  for (i = 0; i < group->n_mem; i++) {
    if (group->mem[i] != NULL) {
      ...
      GST_INFO_OBJECT (allocator, "exported DMABUF[%d] as fd %d",
    ...
    GstAllocator * dmabuf_allocator)
{
  ...
  for (i = 0; i < group->n_mem; i++) {
    ...
    if (group->mem[i] == NULL) {
      ...
      expbuf.index = group->buffer.index;
      expbuf.plane = i;
      expbuf.flags = FD_CLOEXEC | O_RDWR;
