...
void VideoDec_FFMPEG::closeDec()
{
    ...
    is_closed = 1;
    av_log(NULL, AV_LOG_INFO, "#VideoDec_FFMPEG closed \n");
}

const AVCodec* VideoDec_FFMPEG::findBmDecoder(AVCodecID dec_id, const char *name, int codec_name_flag, enum AVMediaType type){
                                      ...
                                      int output_format_mode, int sophon_idx, int pcie_no_copyback)
{
    ...
    char errStr[256] = { 0 };
    sprintf(device_name, "%d\n", sophon_idx);
    ret = av_hwdevice_ctx_create(&hw_device_ctx, AV_HWDEVICE_TYPE_BMCODEC, device_name, NULL, 0);
    if (ret < 0) {
    ...
    *dec_ctx = avcodec_alloc_context3(decoder);// have fix_fmt
    ...
    (*dec_ctx)->get_format = get_bmcodec_format;
...
int VideoDec_FFMPEG::grabFrame2(AVFrame *frame)
{
    int ret = 0;
    if(fflush_flag == 0) {
        while (1) {
    ...
    if(fflush_flag){
        ret = flushDecoder(video_dec_ctx, frame);
    }
...
int VideoDec_FFMPEG::flushDecoder(AVCodecContext *dec_ctx, AVFrame *frame)
{
    static int ret;
    // av_frame_unref(frame);
