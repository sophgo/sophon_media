                              ...
                              int framerate,int width, int height, int encode_pix_format, int bitrate,int sophon_idx)
{
    ...
    enc_ctx->hw_frames_ctx = av_hwframe_ctx_alloc(hw_device_ctx);
    if (!enc_ctx->hw_frames_ctx)
        return AV_PIX_FMT_NONE;
    frames_ctx = (AVHWFramesContext*)enc_ctx->hw_frames_ctx->data;

    frames_ctx->format        = (AVPixelFormat)enc_pix_format;
    ...
    if (!strncmp(codec_name,"bmx264",6)) {
        /* for debug: aq-mode=0:psy=0:mbtree=0 */
       av_dict_set(&dict, "enc-params", "aq-mode=0:b-adapt=0:bframes=1:rc-lookahead=0:ref=2:scenecut=0:subme=2:trellis=0:weightp=0", 0);
    } else {
        /* set private parameters for bm video encoder */
        av_dict_set(&dict, "enc-params", "gop_preset=2:max_qp=44", 0);
...
void VideoEnc_FFMPEG::closeEnc()
{
    ...
    is_closed = 1;
    av_log(NULL, AV_LOG_INFO, "#VideoEnc_FFMPEG closed \n");
}
